...
echo
echo Testing connectivity from $RED"frontend"$NORMAL pod in $YELLOW"default"$NORMAL namespace to $RED"recommendationservice"$NORMAL pod in $YELLOW"default"$NORMAL namespace
echo Expected result should say $GREEN"recommendationservice open"$NORMAL
RESULT2=$(kubectl exec -it $(kubectl -n default get po -l app=frontend -ojsonpath='{.items[0].metadata.name}') -c server -- sh -c 'nc -zv -w 3 recommendationservice 8080')
echo Actual result is $GREEN"$RESULT2"$NORMAL
echo
echo Testing connectivity across namespaces
echo Testing connectivity from $RED"centos"$NORMAL pod in $YELLOW"dev"$NORMAL namespace to $RED"frontend"$NORMAL pod in $YELLOW"default"$NORMAL namespace
echo Expected result should be $GREEN"HTTP/1.1 200 OK"$NORMAL if the traffic is accepted
RESULT3=$(kubectl -n dev exec -t centos -- sh -c 'curl -m3 -sI http://frontend.default 2>/dev/null | grep -i http')
...
echo
echo Testing outbound internet access from $RED"frontend"$NORMAL pod in $YELLOW"default"$NORMAL namespace to $GREEN"www.google.com"$NORMAL
echo Expected result should be $GREEN"HTTP/1.1 200 OK"$NORMAL if the traffic is accepted
